...
class ApiService {
  ...
  static Future<Map<String, String>> getLatestServerUpdates({
    ...
    try {
      final res = await http
          .get(Uri.parse('$baseUrl/updates'), headers: _buildHeaders())
  ...
  static Future<String?> register({
    ...
    try {
      res =
          await _postJsonWithConnectRetry(Uri.parse('$baseUrl/auth/register'), {
  ...
  static Future<OtpRequestResult> requestOtp({
    ...
    http.Response res;
    try {
      res = await _postJsonWithConnectRetry(Uri.parse('$baseUrl/otp/request'), {
  ...
  static Future<OtpVerifyResult> verifyOtp({
    ...
    http.Response res;
    try {
      res = await _postJsonWithConnectRetry(Uri.parse('$baseUrl/otp/verify'), {
        'destination': destination,
        'type': type,
        'purpose': purpose,
        'otp': otp,
  ...
  static Future<OtpRequestResult> requestPhoneAuthOtp({
  ...
  static Future<OtpVerifyResult> verifyPhoneAuthOtp({
    required String phone,
    required String otp,
    String? firstName,
    String? lastName,
    ...
    if (ok) {
      ...
      if (_containsUserPayload(map)) {
        final profile = _extractUserProfile(map, fallbackPhone: phone.trim());
        if (profile != null) {
          await _saveUserProfile(profile);
        }
      }
    ...
    return OtpVerifyResult(
      ok: ok,
      message: _extractApiMessage(map) ?? 'OTP verification completed.',
      token: token,
    );
  }
  ...
  static Future<OtpVerifyResult> verifyFirebasePhone({
    ...
    if (ok) {
      ...
      if (_containsUserPayload(map)) {
        final profile = _extractUserProfile(map, fallbackPhone: destination);
        if (profile != null) {
          await _saveUserProfile(profile);
        }
      }
  ...
  // ================= WARRANTIES =================
  static Future<List<Map<String, dynamic>>> getWarranties({
    String? status,
  }) async {
    final token = await getToken();
  ...
  static Map<String, String> _buildHeaders([
    ...
    final headers = <String, String>{'Accept': 'application/json', ...extra};
  ...
  static Future<OtpRequestResult> requestPhoneChangeOtp(String phone) async {
    ...
    if (!ok &&
        map['errors'] is Map &&
        (map['errors'] as Map)['phone'] is List) {
  ...
  static Future<String?> confirmPhoneChangeOtp({
    ...
    try {
      ...
      if (data is Map &&
          data['errors'] is Map &&
          (data['errors'] as Map)['phone'] is List) {
  ...
  static Future<Product?> fetchProductById(int productId) async {
    ...
    try {
      final res = await http
          .get(
            uri,
            headers: _buildHeaders(const {'Accept': 'application/json'}),
          )
          .timeout(const Duration(seconds: 12));
    ...
    try {
      if (token == null || token.isEmpty) {
        ...
        res = await http
            .get(
              Uri.parse(
                '$baseUrl/guest/notifications',
              ).replace(queryParameters: {'guest_device_id': guestDeviceId}),
              headers: const {'Accept': 'application/json'},
            )
            .timeout(const Duration(seconds: 18));
      } else {
...
class CheckoutOptions {
  const CheckoutOptions({
    ...
    required this.deliveryFeeTiers,
  });
  ...
  /// Mirrors the backend's `DeliveryFeeService::feeForDistanceKm()` so the
  /// fee shown while picking a location matches what the order is charged.
  double feeForDistanceKm(double distanceKm) {
    for (final tier in deliveryFeeTiers) {
      if (distanceKm <= tier.maxKm) return tier.fee;
    }
    if (deliveryFeeTiers.isNotEmpty) return deliveryFeeTiers.last.fee;
    return deliveryFee;
  }
  ...
  factory CheckoutOptions.fromApi(Map<String, dynamic> map) {
    ...
    final rawShopLocation = data['shop_location'];
    final shopLocation = rawShopLocation is Map
        ? ShopLocation.fromApi(Map<String, dynamic>.from(rawShopLocation))
        : ShopLocation.fallback;

    final rawTiers = data['delivery_fee_tiers'];
    final deliveryFeeTiers = rawTiers is List
        ? rawTiers
              .whereType<Map>()
              .map(
                (item) =>
                    DeliveryFeeTier.fromApi(Map<String, dynamic>.from(item)),
              )
              .toList()
        : <DeliveryFeeTier>[];
    ...
    return CheckoutOptions(
      ...
      deliveryFeeTiers: deliveryFeeTiers.isEmpty
          ? DeliveryFeeTier.fallbackList()
          : deliveryFeeTiers,
    );
  }
  ...
  factory CheckoutOptions.fallback() {
    return CheckoutOptions(
      ...
      shopLocation: ShopLocation.fallback,
      deliveryFeeTiers: DeliveryFeeTier.fallbackList(),
    );
  }
...
class ShopLocation {
  ...
  factory ShopLocation.fromApi(Map<String, dynamic> map) {
    final name = map['name']?.toString().trim();
    return ShopLocation(
      name: (name == null || name.isEmpty) ? 'KneaYerng VIP' : name,
      lat: CheckoutOptions._toDouble(map['lat']) ?? fallback.lat,
      lng: CheckoutOptions._toDouble(map['lng']) ?? fallback.lng,
    );
  }

  static const ShopLocation fallback = ShopLocation(
    name: 'KneaYerng VIP',
    lat: 11.5664368,
    lng: 104.8923294,
  );
...
class DeliveryFeeTier {
  ...
  factory DeliveryFeeTier.fromApi(Map<String, dynamic> map) {
    return DeliveryFeeTier(
      maxKm: CheckoutOptions._toDouble(map['max_km']) ?? 0,
      fee: CheckoutOptions._toDouble(map['fee']) ?? 0,
    );
  }

  static List<DeliveryFeeTier> fallbackList() {
    return const [
